...
public class NetBeansJavaProjectElementUtils {
    ...
    public static ElementHandle findEnclosingElementHandle(Project project, ElementHandle handle) {
        ...
        EnclosingElementHandleSearcher searcher = new EnclosingElementHandleSearcher(handle);
        try {
    ...
    public static Name getSimpleName(ElementHandle handle, Project project) {
        if (!CLASSPATH_INFO.containsKey(project)) {
            CLASSPATH_INFO.put(project, getClasspathInfo(project));
        }
        if (!JAVA_SOURCE.containsKey(project)) {
    ...
    private static class EnclosingElementHandleSearcher implements CancellableTask<CompilationController> {

        private ElementHandle element;
        private final ElementHandle constructor;

        public EnclosingElementHandleSearcher(ElementHandle constructor) {
        ...
        public void run(CompilationController info) throws Exception {
            ...
            Element elem = constructor.resolve(info);
            if (elem != null) {
                Element enclosing = elem.getEnclosingElement();
                element = ElementHandle.create(enclosing);
            }
        }
    ...
    private static class SimpleNameSearcher implements CancellableTask<CompilationController> {
